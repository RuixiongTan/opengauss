...
read -p "Please input OG_SUBNET (容器所在网段) [172.15.0.0/24]: " OG_SUBNET
OG_SUBNET=${OG_SUBNET:-172.15.0.0/24}
...
read -p "Please input SHARED_DATA_DIR（共享数据目录）[/mnt/trxB/]" SHARED_DATA_DIR
SHARED_DATA_DIR=${SHARED_DATA_DIR:-/mnt/trxB/}
echo "SHARED_DATA_DIR set $SHARED_DATA_DIR"

read -p "Please input network name (容器网络名字)[opengaussnetwork_trxB]: " NETWORK_NAME
NETWORK_NAME=${NETWORK_NAME:-opengaussnetwork_trxB}
echo "NETWORK_NAME set $NETWORK_NAME"

read -p "Please input MASTER_IP (主库IP)[172.15.0.101]: " MASTER_IP
MASTER_IP=${MASTER_IP:-172.15.0.101}
echo "MASTER_IP set $MASTER_IP"

read -p "Please input SLAVE_1_IP (备库IP)[172.15.0.102]: " SLAVE_1_IP
SLAVE_1_IP=${SLAVE_1_IP:-172.15.0.102}
echo "SLAVE_1_IP set $SLAVE_1_IP"

read -p "Please input MASTER_HOST_PORT (主库数据库服务端口)[7632]: " MASTER_HOST_PORT
MASTER_HOST_PORT=${MASTER_HOST_PORT:-7632}
echo "MASTER_HOST_PORT set $MASTER_HOST_PORT"

read -p "Please input MASTER_LOCAL_PORT (主库通信端口)[7634]: " MASTER_LOCAL_PORT
MASTER_LOCAL_PORT=${MASTER_LOCAL_PORT:-7634}
echo "MASTER_LOCAL_PORT set $MASTER_LOCAL_PORT"

read -p "Please input MASTER_CONFIG_PATH（主库配置文件路径）[/mnt/trxB/og1]" MASTER_CONFIG_PATH
MASTER_CONFIG_PATH=${MASTER_CONFIG_PATH:-/mnt/trxB/og1}
echo "MASTER_CONFIG_PATH set $MASTER_CONFIG_PATH"

read -p "Please input SLAVE_1_HOST_PORT (备库数据库服务端口)[8632]: " SLAVE_1_HOST_PORT
SLAVE_1_HOST_PORT=${SLAVE_1_HOST_PORT:-8632}
echo "SLAVE_1_HOST_PORT set $SLAVE_1_HOST_PORT"

read -p "Please input SLAVE_1_LOCAL_PORT (备库通信端口)[8634]: " SLAVE_1_LOCAL_PORT
SLAVE_1_LOCAL_PORT=${SLAVE_1_LOCAL_PORT:-8634}
echo "SLAVE_1_LOCAL_PORT set $SLAVE_1_LOCAL_PORT"

read -p "Please input SLAVE_1_CONFIG_PATH（备库配置文件路径）[/mnt/trxB/og2]" SLAVE_1_CONFIG_PATH
SLAVE_1_CONFIG_PATH=${SLAVE_1_CONFIG_PATH:-/mnt/trxB/og2}
echo "SLAVE_1_CONFIG_PATH set $SLAVE_1_CONFIG_PATH"

read -p "Please input MASTER_NODENAME [opengauss_master_trxB]: " MASTER_NODENAME
MASTER_NODENAME=${MASTER_NODENAME:-opengauss_master_trxB}
echo "MASTER_NODENAME set $MASTER_NODENAME"

read -p "Please input SLAVE_NODENAME [opengauss_slave1_trxB]: " SLAVE_NODENAME
SLAVE_NODENAME=${SLAVE_NODENAME:-opengauss_slave1_trxB}
